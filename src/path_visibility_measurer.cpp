...
static const double PATH_LENGTH = 3.0;
...
private:
    void startMeasuring(const position_tracker::StartMeasurementConstPtr msg) {
        ...
        leftCameraSub->registerCallback(boost::bind(&PathVisibilityMeasurer::callback, this, _1));
    ...
    void callback( const CameraInfoConstPtr& lCameraInfo) {
        ...
        pathRectInBaseFrame->points[0] = createPoint(BASE_WIDTH / 2.0, -BASE_WIDTH / 2.0, 0);
        pathRectInBaseFrame->points[1] = createPoint(BASE_WIDTH / 2.0, BASE_WIDTH / 2.0, 0);
        ...
        cv::Point3d centerPoint = cameraModel.projectPixelTo3dRay(cv::Point(0, 0));
