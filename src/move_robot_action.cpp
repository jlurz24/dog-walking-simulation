...
public:
    MoveRobotAction(const string& name) :
        as(nh, name, boost::bind(&MoveRobotAction::move, this, _1), false), actionName(name) {
        ...
        getEntireRobotPathClient = nh.serviceClient<dogsim::GetEntireRobotPath>("/dogsim/get_entire_robot_path", true /* persist */);


        // Initialize the command thread
        commandThread = unique_ptr < boost::thread
                        > (new boost::thread(
                                boost::bind(&MoveRobotAction::execute, this)));
    }
    ~MoveRobotAction(){
...
protected:
    ...
    void execute() {
        ...
        while (true) {
            ...
            assert(getPath.response.poses.size() > 0 && getPath.response.poses[0].header.frame_id == "/map");
            ...
            for (unsigned int i = 0; i < getPath.response.poses.size(); ++i) {
                if (getPath.response.poses[i].header.stamp > now) {
                    absoluteGoal = getPath.response.poses[i];
                }
            }
            if (!found) {
                ROS_INFO("Could not locate any future goals");
                boost::this_thread::sleep(CYCLE_TIME);
                continue;
            }
            ...
            try {
                tf.transformPose("/base_footprint", ros::Time(0), absoluteGoal,
                        absoluteGoal.header.frame_id, goalPose);
            }
            catch (tf::TransformException& ex) {
                ROS_ERROR("Failed to transform absolute goal from %s to /base_footprint frame",
                        absoluteGoal.header.frame_id.c_str());
                continue;
            }
            ...
            double totalDistance = utils::pointToPointXYDistance(goalPose.pose.position,
                    robotPose.position);
            ...
            while (ros::ok() && active
                    && (currentDistance > DISTANCE_THRESHOLD || qDistance > Q_DISTANCE_THRESHOLD)) {
                ...
                if (totalDistance > DISTANCE_THRESHOLD) {
                    ratio = min(
                            (totalDistance
                                    - (currentDistance
                                            - MAX_LINEAR_V * 1.0 / static_cast<double>(HZ)))
                                            / totalDistance, 1.0);
                }
                ROS_DEBUG("Total distance %f current distance %f ratio %f", totalDistance,
                ...
                if (goalPose.pose.position.x < 0 && currentDistance > DISTANCE_THRESHOLD
                        && fabs(yaw > 3 * PI / 4)) {
                    if (currentDistance < MAX_REVERSE_DISTANCE) {
                        ROS_INFO(
                                "Moving in reverse at original anglular velocity %f at velocity %f",
                                yaw, baseCmd.linear.x);
                        if (yaw > PI / 2.0) {
                            yaw -= PI;
                        }
                        else {
                        ...
                        baseCmd.linear.x *= -1;
                        ROS_INFO("Moving in reverse at anglular velocity %f at velocity %f", yaw,
                                baseCmd.linear.x);
                    }
                    else {
                        // Use yaw as is.
                ...
                else if (calcQDistance(tf::createIdentityQuaternion(), interp)
                        < Q_DISTANCE_THRESHOLD) {
                    ROS_DEBUG(
                            "Not performing turn because threshold reached. yaw = %f qDistance = %f",
                            yaw, calcQDistance(tf::createIdentityQuaternion(), interp));
                    // Turning slows the robot down, so avoid turning when possible.
                    yaw = 0;
                }
                // Turn and drive
                else {
                    ROS_DEBUG("Turning and driving. Yaw = %f qDistance = %f", yaw,
                            calcQDistance(tf::createIdentityQuaternion(), interp));
                    // Use yaw as is.
                }
                ...
                ROS_DEBUG("Moving base x: %f, y: %f, angular z: %f", baseCmd.linear.x,
                ...
                if (nh.ok() && active) {
                    ...
                    try {
                        ...
                        ROS_ERROR(
                                "Failed to transform absolute goal from %s to /base_footprint frame",
                                absoluteGoal.header.frame_id.c_str());
                        // Continue and hope that it transforms next time.
                    }
                    currentDistance = utils::pointToPointXYDistance(goalPose.pose.position,
                            robotPose.position);
                    tf::quaternionMsgToTF(goalPose.pose.orientation, goalPoseTF);
...
private:
    ...
    std::unique_ptr<boost::thread> commandThread;
