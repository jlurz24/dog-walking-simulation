...
public:
	...
	void Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) {
		...
		getPathClient = nh.serviceClient<dogsim::GetPath>("/dogsim/get_path",
				true);
...
private:
	...
	void startPath() {
		...
		ros::ServiceClient startPathClient =
				nh.serviceClient<dogsim::StartPath>("/dogsim/start", false);
	...
	void initGaussians() {
		...
		ros::ServiceClient maxTimeClient =
				nh.serviceClient<dogsim::MaximumTime>("/dogsim/maximum_time",
						false);
		...
		ROS_INFO(
				"Gaussian parameters -  pNewGauss: %f gaussHeight: %f gaussMinWidth: %f gaussMaxWidth: %f", pNewGauss, gaussHeight, gaussMinWidth, gaussMaxWidth);

		ROS_INFO(
				"Initializing gaussians. Maximum time is %f", maxTime.response.maximumTime.toSec());
		...
		for (double t = 0; t <= maxTime.response.maximumTime.toSec();
			...
			if (randomZeroToOne(rng)
					< pNewGauss
							/ static_cast<double>(SIMULATOR_CYCLES_PER_SECOND)) {
				// Create a structure with the parameters.
				GaussParams params;
				boost::uniform_real<> randomA(-gaussHeight * pi,
						gaussHeight * pi);
				params.a = randomA(rng);

				boost::uniform_real<> randomC(gaussMinWidth * pi,
						gaussMaxWidth * pi);
				params.c = randomC(rng);

				ROS_DEBUG(
						"New gaussian created with a: %f c: %f at time %f", params.a, params.c, t);
				params.startTime = t;
				gaussParams.push_back(params);
			}
		}
		ROS_INFO(
				"Completed initializing gaussians. Total gaussians is %lu", gaussParams.size());
	}
	...
	void OnUpdate() {
		...
		double liftFactor = min(
				log(10 * this->model->GetWorldPose().pos.z) / log(10 * 0.05),
				1.0);
		ROS_DEBUG(
				"LIFT_FACTOR %f @ height %f", liftFactor, this->model->GetWorldPose().pos.z);
		...
		if (int(currTime.Double() * 1000) % (1000 / PUBLISH_FREQUENCY) == 0
			...
			dogGoalVizPub.publish(
					utils::createMarker(goalPoint.point, goalPoint.header, RED,
							true));
		}
		...
		const double errorDerivativeX = calcErrorDerivative(
				this->previousErrorX, errorX, deltat);
		const double errorDerivativeY = calcErrorDerivative(
				this->previousErrorY, errorY, deltat);
		...
		body->AddForce(
				math::Vector3(this->forceX * liftFactor,
						this->forceY * liftFactor, 0.0));
...
private:
	static double calcError(const double _currentPosition,
			const double _goalPosition) {
		return _goalPosition - _currentPosition;
	}

	static double calcErrorDerivative(const double _previousError,
			const double _currentError, const common::Time& _deltaTime) {
	...
	math::Vector3 addGaussians(const math::Vector3& base,
			const math::Vector3& previousBase, const double t) {
		...
		for (unsigned int i = 0; i < gaussParams.size(); ++i) {
			...
			double gx = t - gaussParams[i].startTime
					- GAUSS_HALF_WIDTH * gaussParams[i].c;
			double gy = gaussParams[i].a
					* exp(
							-(utils::square(gx)
									/ (2 * utils::square(gaussParams[i].c))));
