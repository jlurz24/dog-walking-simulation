...
public:
	...
	DogPositionDetector() :
		...
		ros::SubscriberStatusCallback connectCB = boost::bind(
				&DogPositionDetector::startListening, this);
		ros::SubscriberStatusCallback disconnectCB = boost::bind(
				&DogPositionDetector::stopListening, this);

		dogPositionPub = nh.advertise<DogPosition>(
				"/dog_position_detector/dog_position", 1, connectCB, disconnectCB);
		objectSub.registerCallback(
				boost::bind(&DogPositionDetector::callback, this, _1));
...
private:
	...
	void callback(const position_tracker::DetectedDynamicObjectsConstPtr msg) {
		...
		dogPositionMsg.pose.header = msg->header;
		if (msg->positions.size() == 0) {
			...
			ROS_INFO(
					"Dog position message has %lu positions", msg->positions.size());
