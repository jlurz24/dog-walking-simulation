...
public:
  ...
  RobotDriver(): pnh("~"), adjustDogClient("adjust_dog_position_action", true),
    ...
    getPlannedRobotPoseClient = nh.serviceClient<GetPlannedRobotPose>("/dogsim/get_planned_robot_pose", true);
  ...
  void dogPositionCallback(const DogPositionConstPtr& dogPosition){
    ...
    if(dogPosition->unknown){
    	...
    	if(searchForDogClient.getState() == actionlib::SimpleClientGoalState::ACTIVE){
    		ROS_INFO("Search for dog already ongoing.");
    		return;
    	}
  ...
  void steeringCallback(const ros::TimerEvent& event){
      ...
      getPlannedPose.request.time = event.current_real + ros::Duration(MOVE_ROBOT_UPDATE_INTERVAL);
      ...
      // TODO: Point the robot's head at the path.
